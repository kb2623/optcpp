...
void jSOp::run_thread(size_t tid) {
	...
	while (!stop_cond(*this)) {
		for (int i = s * thread_td->tid; i < pop.size() && i < s * (thread_td->tid + 1); i++) sorted_array[i] = i;
		for (int i = s * thread_td->tid; i < pop.size() && i < s * (thread_td->tid + 1); i++) temp_fit[i] = fitness[i];
		sync->arrive_and_wait();
		if (thread_td->tid == 0) sortIndexWithQuickSort(temp_fit, 0, pop.size() - 1, sorted_array);
		sync->arrive_and_wait();
		for (int target = s * thread_td->tid; target < pop.size() && target < s * (thread_td->tid + 1); target++) {
		...
		sync->arrive_and_wait();
		for (int i = s * thread_td->tid; i < pop.size() && i < s * (thread_td->tid + 1); i++) {
